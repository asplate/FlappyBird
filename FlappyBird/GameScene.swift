...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            //サウンド再生
            let getItemSound = SKAction.playSoundFileNamed("se_get_item.mp3",waitForCompletion: false)
            self.run(getItemSound,completion: ({
            ...
            //サウンド再生
            let gameoverSound = SKAction.playSoundFileNamed("se_gameover.mp3",waitForCompletion: false)
            self.run(gameoverSound,completion: ({
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width - 50 + itemTexture.size().width / 2, y: 0.0)
            ...
            // 上側の壁を作成
            let itemCont = SKSpriteNode(texture: itemTexture)
            itemCont.position = CGPoint(x: 0.0, y: under_item_y + itemTexture.size().height)
            
            
            // スプライトに物理演算を設定する
            itemCont.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
            itemCont.physicsBody?.categoryBitMask = self.itemCategory
            
            // 衝突の時に動かないように設定する
            itemCont.physicsBody?.isDynamic = false
            
            item.addChild(itemCont)
            
            
            // スコアアップ用のノード
            let itemScoreNode = SKNode()
            itemScoreNode.position = CGPoint(x: itemCont.size.width + self.bird.size.width / 2, y: self.frame.height / 2.0)
            itemScoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: itemCont.size.width, height: self.frame.size.height))
